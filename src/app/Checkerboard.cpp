...
void VkexInfoApp::CheckerboardUpscale(vkex::CommandBuffer cmd, uint32_t frame_index)
{
    ...
    m_generated_shader_states[AppShaderList::CheckerboardUpscale].descriptor_sets[frame_index]->
        UpdateDescriptor(1, m_checkerboard_simple_render_pass[kLeftCBIndex].rtv_texture);
    m_generated_shader_states[AppShaderList::CheckerboardUpscale].descriptor_sets[frame_index]->
        UpdateDescriptor(2, m_checkerboard_simple_render_pass[kRightCBIndex].rtv_texture);

    cmd->CmdTransitionImageLayout(m_checkerboard_simple_render_pass[cb_frame_index].rtv_texture,
        VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
        VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT);
    cmd->CmdTransitionImageLayout(m_checkerboard_simple_render_pass[other_cb_index].rtv_texture,
        VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
        VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT);
    ...
    std::vector<uint32_t> dynamic_offsets = { checkerboard_constants_dynamic_offset };
    cmd->CmdBindDescriptorSets(
        VK_PIPELINE_BIND_POINT_COMPUTE,
        *(m_generated_shader_states[AppShaderList::CheckerboardUpscale]
            .pipeline_layout),
        0, { *(m_generated_shader_states
                  [AppShaderList::CheckerboardUpscale]
