...

{
  ...
  for (size_t samplerIndex = 0; samplerIndex < samplerCount; samplerIndex++) {
    ...
    VKEX_CALL(queue->GetDevice()->CreateSampler(create_info,
...

{
  ...
  for (size_t matIndex = 0; matIndex < matCount; matIndex++) {
    ...
    VKEX_ASSERT(source_material.pbrMetallicRoughness.baseColorTexture.index >=
                0);
    m_materials[matIndex].baseColorTextureIndex =
    ...
    for (size_t factor_index = 0;
         factor_index < source_material.emissiveFactor.size(); factor_index++) {
...

{
  ...
  for (size_t imageIndex = 0; imageIndex < imageCount; imageIndex++) {
    ...
    asset_util::CreateTexture(
        sourceImage.image.size(), &(sourceImage.image.at(0)), sourceImage.width,
        sourceImage.height, texture_format, queue,
        asset_util::MEMORY_USAGE_GPU_ONLY, &(m_images[imageIndex].gpuTexture));
  }
}

{
  for (auto& material : m_materials) {
    ...
    material.textures[MaterialTextureType::BaseColor] =
        m_images[baseColorTexture.imageIndex].gpuTexture;
    material.samplers[MaterialTextureType::BaseColor] =
        m_samplers[baseColorTexture.samplerIndex];

    const auto& metallicRoughnessTexture =
        m_textures[material.metallicRoughnessTextureIndex];
    material.textures[MaterialTextureType::MetallicRoughness] =
        m_images[metallicRoughnessTexture.imageIndex].gpuTexture;
    material.samplers[MaterialTextureType::MetallicRoughness] =
        m_samplers[metallicRoughnessTexture.samplerIndex];

    const auto& emissiveTexture = m_textures[material.emissiveTextureIndex];
    material.textures[MaterialTextureType::Emissive] =
        m_images[emissiveTexture.imageIndex].gpuTexture;
    material.samplers[MaterialTextureType::Emissive] =
        m_samplers[emissiveTexture.samplerIndex];

    const auto& occlusionTexture = m_textures[material.occlusionTextureIndex];
    material.textures[MaterialTextureType::Occlusion] =
        m_images[occlusionTexture.imageIndex].gpuTexture;
    material.samplers[MaterialTextureType::Occlusion] =
        m_samplers[occlusionTexture.samplerIndex];

    const auto& normalTexture = m_textures[material.normalTextureIndex];
    material.textures[MaterialTextureType::NormalTex] =
        m_images[normalTexture.imageIndex].gpuTexture;
    material.samplers[MaterialTextureType::NormalTex] =
        m_samplers[normalTexture.samplerIndex];
  }
}
}
...
const std::vector<vkex::Texture>& GLTFModel::GetMaterialTextures(
    uint32_t node_index, uint32_t primitive_index) {
  const auto& prim = GetPrimitive(node_index, primitive_index);
...
const std::vector<vkex::Sampler>& GLTFModel::GetMaterialSamplers(
    uint32_t node_index, uint32_t primitive_index) {
  const auto& prim = GetPrimitive(node_index, primitive_index);
  const auto& material = m_materials[prim.materialIndex];
